...
namespace tldr_Discord
{
    public class TLDRCommands : SlashCommandModule
    {
            ...
            )
        {
            ...
            //language ??= ctx.Guild.PreferredLocale;
                ...
                var embed = new DiscordEmbedBuilder()
                    ...
                    .WithFooter("tldr - collaborative cheatsheets for console commands ", "https://github.githubassets.com/images/icons/emoji/unicode/1f4da.png")
        ...
        string? FindPage(string page, string platformPref)
        {
            ...
            var platfromPaths = Directory.GetDirectories(rootDir).Select(x => x.Split('/').LastOrDefault()).Where(x => x != "common" || x != platformPref);
            
            foreach (var platfromPath in platfromPaths)
            {
                var ppage = GetPageInPath(page, platfromPath);

                if (File.Exists(ppage))
                {
                    return ppage;
                }
        ...
        string? GetPageInPath(string page, string platform)
        {
            var rootDir = Path.Combine("tldr-cache", "pages");
